<div class="antialiased bg-gray-200">
    <div class="container mx-auto px-4 sm:px-8">
        <div class="py-8">
            <div>
                <h2 class="text-2xl capitalize font-semibold leading-tight">{{ __('symptoms') }}</h2>
            </div>
            <div class="my-2 flex sm:flex-row flex-col">
                <div class="flex flex-row mb-1 sm:mb-0 justify-center">
                    <div class=" w-72 relative me-1 ">
                            <label for="simple-search" class="sr-only">{{('Search Symptom')}}</label>
                                ...
                                <tr>
                                    ...
                                    <td width="10%" class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <div class="flex sm:flex-wrap justify-end items-center gap-1">
                                            <x-button.circle spinner="edit" loading-delay="short" wire:click="edit({{$symptom->id}})" primary icon="pencil" />
                                            <x-button.circle spinner="delete" loading-delay="short" wire:click="delete({{$symptom->id}})" negative icon="trash" />
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
    ...
    {{-- Modal Crear --}}
    <x-modal.card title="{{ __('add symptom') }}" blur wire:model="modal">
        <div class="grid grid-cols-1 sm:grid-cols-1 gap-4">
            ...
            <x-input wire:model="name" label="{{ __('Name') }}"
                placeholder="{{ __('name of the symptom..') }}" />
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                <div class="flex">
                    ...
                    <x-button primary label="{{ __('create') }}" wire:click="addSymptom" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
    ...
    {{-- Modal Editar --}}
    <x-modal.card title="{{ __('edit symptom') }}" blur wire:model="modalEdit">
        <div class="grid grid-cols-1 sm:grid-cols-1 gap-4">
            <div class="col-span-1 sm:col-span-2">
                <x-badge flat lg primary label="Doc: {{ auth()->user()->name }}" />
            </div>
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                <div class="flex">
                    ...
                    <x-button spinner="update" loading-delay="short"  primary icon="check" label="{{ __('update') }}" wire:click="update({{ $symptomId }})" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
