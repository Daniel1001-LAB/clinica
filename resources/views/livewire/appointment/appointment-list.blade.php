                        ...
                        <div class="">
                            <div class="flex items-center space-x-4 bg-slate-200 p-4">
                                <div class="flex-shrink-0">
                                    <img class="w-10 h-10 rounded-full"
                                        src="{{ $appoinment->patient->profile_photo_url }}"
                            ...
                            <div class="flex flex-col bg-white rounded-lg p-4">
                                <div class="flex items-center justify-between mb-4">
                                    <h2 class="text-lg font-bold text-gray-900 capitalize">{{ __('opening hours') }}
                                    </h2>
                                ...
                                <div
                                    class="flex flex-col mt-4 md:flex-row md:justify-between md:items-center md:gap-x-4">
                                    <div class="flex justify-center md:justify-start mb-2 md:mb-0">
                                        <x-button spinner wire:click="canceled({{ $appoinment->id }})" xs
                                            icon="x" negative label="{{ __('cancel') }}" :disabled="$appoinment->status !== 'PENDING'" />
                                    </div>
                                    <div class="flex justify-center md:justify-end">
                                        <x-button spinner wire:click="confirmed({{ $appoinment->id }})" xs
                                            icon="check" positive label="{{ __('confirm') }}" :disabled="$appoinment->status !== 'PENDING'" />

                                        <x-button wire:click="accomplished({{ $appoinment->id }})" class="ms-4"
                                            spinner xs icon="chevron-up" primary label="{{ __('go to interview') }}"
