<div class="p-2">
    <div class="flex items-center">
        ...
        <div class="relative w-full">
            ...
            <input type="text" id="simple-search " wire:model="search"
                ...
                placeholder="{{__('Search surgeries')}}" required>
        </div>
    </div>

    <div class="pt-2">
        <h2 class="mb-2 text-sm font-semibold capitalize text-gray-900 dark:text-white">{{ __('registered surgeries') }}</h2>
        <ul class="w-full text-center space-y-1 text-gray-500 list-inside dark:text-gray-400">
            @forelse ($patient_surgeries as $patient_surgery)
            <li class="grid grid-cols-1 w-full items-center shadow-sm font-semibold ">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <a class="cursor-pointer"> {{ $patient_surgery->name }}</a>
                    <x-button.circle 2xs negative icon="x"
                        wire:click="removeSurgery({{ $patient_surgery->id }})" />
                </div>
            </li>
            @empty
            <div class="justify-center">
                <x-badge icon="clipboard-list" md flat primary label="{{ 'no registered surgeries' }}" />
            </div>
            @endforelse
        ...
        <ul class="mb-8 mt-3  space-y-4 text-center text-gray-500 dark:text-gray-400">
            @forelse ($surgeries as  $surgery)
                <li class="flex items-center space-x-3">
                    ...
                    <a wire:click="addModalSurgery({{ $surgery->id }})" class="cursor-pointer">{{ $surgery->name }}</a>
                </li>
            ...
            @empty
                ...
                @if (strlen(trim($this->search)) > 8)
                    <div class="justify-center">
                        <x-badge icon="clipboard-list" lg flat primary label="{{ 'no search result' }}" />
    ...
    <x-modal.card title="{{ __('add surgery to patient history') }}" blur wire:model="modal">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="col-span-2 sm:col-span-2">
                <x-input wire:model="name" label="{{ 'Name' }}" placeholder="{{ 'name of the surgery' }}" />
            </div>
            <x-inputs.number wire:model="year" label="{{ 'Year' }}" placeholder="{{ 'year of the surgery' }}" />
        ...
        <x-slot name="footer">
            <div class="flex justify-between items-end gap-x-4">
                ...
                <x-button icon="check" primary label="{{ __('add surgery') }}" wire:click="addSurgery" />
            </div>
        </x-slot>
    </x-modal.card>
