<section class="bg-white z-1 rounded max-w-2xl mx-auto p-8">
    <table id="tpatologias" class="row-border text-sm" style="width:100%">
        <thead>
            <tr>
                <th>Patologías</th>
            </tr>
        ...
        <tbody>
            @foreach (auth()->user()->pathologies as $a)
                <tr>
                    <th class="px-6 py-4 font-normal text-gray-900 text-center">
                        <div class="text-sm">
                            <div class="font-medium text-gray-700">{{ $a->name }}</div>
                            <div class="font-medium text-gray-400">{{ date('d-m-Y', strtotime($a->created_at))}}</div>
                        </div>
                    </th>
                </tr>
            @endforeach
